...
%type <d> NUMBER FLOAT  
%type <s> NAME augassign PLUSEQUAL MINEQUAL STAREQUAL SLASHEQUAL PERCENTEQUAL AMPEREQUAL VBAREQUAL CIRCUMFLEXEQUAL LEFTSHIFTEQUAL RIGHTSHIFTEQUAL 
			DOUBLESTAREQUAL DOUBLESLASHEQUAL
//%type <s> stmt plus_stmt simple_stmt compound_stmt small_stmt small_stmt_STAR_OR_SEMI
%type <ast> test star_COMMA_test COMMA testlist opt_arglist arglist opt_test arith_expr term factor power atom opt_yield_test pick_yield_expr_testlist 
			...
			STRING DOUBLESTAR STAR EQUAL LPAR fplist RPAR NOT comparison expr xor_expr and_expr shift_expr funcdef1
%type <c> pick_PLUS_MINUS pick_multop pick_unop TILDE
...
funcdef // Used in: decorated, compound_stmt
	//: DEF NAME parameters COLON suite { std::cout<<"funcdef: NAME= "<<$2<<std::endl;}
  : funcdef1 suite {
 ...
 funcdef1 // Used in: decorated, compound_stmt
...
expr_stmt // Used in: small_stmt
	...
	| testlist star_EQUAL {	
      ...
      //std::cout<<"testlist STAR_EQUAL scope of pSymT "<<pSymT->GetScope()<<std::endl;
        ...
        std::cout<<" $2 type = "<<$2->getNodetype()<<std::endl;
        std::cout<<" $1 type = "<<$1->getNodetype()<<std::endl;
        }

      
			
        if(NULL != pSymT) 
          pSymT->insertDSymbol($1,$2);
...
print_stmt // Used in: small_stmt
    ...
    { 
      ...
      setScope(scope);
		  AstPrintNode *result= new AstPrintNode('p',$2);
...
suite // Used in: funcdef, if_stmt, star_ELIF, while_stmt, for_stmt, 
	...
	: simple_stmt
	| NEWLINE INDENT{ ++scope;TMngr.SetCurrentScope(scope); AstFuncNode *pFuncNode = new AstFuncNode('f',scope,fName); TMngr.SetFuncNode(pFuncNode,fName); } plus_stmt DEDENT {/*std::cout<<"scope changed"<<--scope<<std::endl;*/--scope; TMngr.SetCurrentScope(scope);}
	;
plus_stmt // Used in: suite, plus_stmt
	: stmt plus_stmt { /*std::cout<<"In plus_stmt: stmt plus_stmt"<<std::endl;*/}
	| stmt { /*std::cout<<"In plus_stmt: stmt"<<std::endl;*/}
	;
	...
	| term pick_multop factor
		{ 
			if('*' == $2){
				...
				$$ = new AstMultNode('*',$1,$3); //$1*$3;
			}
			
			if('/' == $2)
			{ 
				//std::cout<<"Division"<<std::endl;
				$$ = new AstDivNode('/',$1,$3); //floor((float)$1/(float)$3);
			}
			...
			if('%' == $2)
			{
				...
				if( 0 != $3)
					$$ = new AstModNode('%', $1, $3);	//(floor)((float)$1 - (floor((float)$1/(float)$3)) * (float)$3);// TODO Change to MOD
				else{
			...
			if('D' == $2)
			{
				...
				$$ = new AstIntDivNode('D', $1,$3);	//floor((float)$1/(float)$3);
			}
...
factor // Used in: term, factor, power
	...
	{ 
	  if('+' == $1)
	  {
		$$ = new AstUnPlusNode('P',$2);
	  }
	  if('-' == $1)
		$$ = new AstUnMinusNode('M',$2);
	}
	| power
	;
...
atom // Used in: power
	...
	| NAME   { $$ = new AstSymbol('S', $1); FCName = $1; /*std::cout<<"Name: "<<$1<<std::endl;*/} //todo: build  the symbol node here and check  
	| FLOAT  { $$ = new AstNumber('F', $1); /*std::cout<<"float: "<<$1<<std::endl;*/}
	| NUMBER { $$ = new AstNumber('I', $1); /*std::cout<<"int: "<<$1<<std::endl;*/}
	| plus_STRING
	;
